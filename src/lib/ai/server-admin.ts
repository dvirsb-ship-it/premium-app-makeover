...
const DAILY_CAPS: Record<string, number> = {
  ...
  detectRegions: 30,
  /*
   * הבעת עניין — תקרה על **ההתראה ללקוח**, לא על העלות (18/8/2026).
   *
   * עורך דין מאושר יכול היה להביע עניין בכל תיק בפיד, כל יום, וכל
   * אחת מהן שולחת לפונה "עורך דין רוצה לקחת את התיק שלך". זו ההצפה
   * שפוגעת בלקוח ישירות — ההצעה עצמה תופסת מקום אחד מעשרה ולכן אינה
   * חוסמת, אבל ההתראה מגיעה לכל אחד ואחד.
   *
   * ארבעים ביום נדיב מאוד לעורך דין אמיתי — הפיד מוגבל ממילא ל-30
   * יום — ומונע הצפה לחלוטין. מי שיחרוג ימשיך להביע עניין, הפונה
   * פשוט לא יקבל התראה, וזה מופיע בלוג השגיאות.
   */
  notifyInterest: 40,
};
