 ...
 * ═══ שלושת מנועי ההערכה ═══
 *
 * ב-10/8/2026 החלפתי אותם במנוע מיון שרק מסווג לתחום, אחרי שזיהיתי
 * שהם פוסקים שאלות משפטיות. **דביר החזיר אותם, ובצדק:** ההערכה היא
 * המוצר. פלטפורמה שרק אוספת מידע אינה שווה דבר לעורך דין, והתקציר
 * המשפטי הוא בדיוק מה שהופך פנייה מליד גולמי לתיק ששווה לקרוא.
 *
 * מה שכן השתנה, ונשאר: **איך זה מוצג**. המערכת מעריכה סיכוי — היא
 * אינה מבטיחה תוצאה, אינה נותנת ייעוץ, והמילים בממשק אומרות את זה.
 * ההערכה הסופית היא של עורך הדין, וזה כתוב בכל מקום שבו ההערכה
 * מוצגת.
 */
...
2. **מועדים** — לערר ולהשגה מועדים קצרים ומחייבים; אם חלף המועד ציין זאת במפורש ב-caseContext.
...
5. caseContext — לעיני עורכי דין: הדין הרלוונטי, השלב, המועדים והמסלול. בתביעה אזרחית — העילה, הדין וההתיישנות. בפלילי — העבירה הנטענת והשלב הדיוני. במשפחה — סוג ההליך והערכאה. מול רשות — ההחלטה הנתקפת והמועד. אם validated=false — הסבר תמציתי של הכשל.
...
- סימונים כמו [מספר זהות הוסר] או [טלפון הוסר] הם הסרה יזומה שלנו להגנת פרטיות, לא מידע חסר. אין לראות בהם פגם בפנייה ואין לפסול תיק בגללם.
    ...
    await notify(
      ...
      result.validated
        ? {
            ...
            body: notified > 0
              ? `"${result.title}" עבר — ${notified} עורכי דין בתחום קיבלו התראה`
