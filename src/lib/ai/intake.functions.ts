...
/*
 * ═══ מיון לתחום — לא הכרעה לגופו של עניין (10/8/2026) ═══
 *
 * הגרסה הקודמת הייתה "מנוע הוולידציה **המשפטית**", והיא הכריעה בארבעה
 * צירים: עילה, התיישנות, נזק ומסלול. היא החזיקה טבלת מועדים (נזיקין 7
 * שנים, ביטוח 3) ועל פיה קבעה `validated=false` — כלומר **דחתה אדם**
 * בנימוק שהתיק שלו התיישן או שאין לו עילה.
 *
 * זו חוות דעת משפטית של מי שאינו עורך דין. וגרוע מכך — הטעות בה אינה
 * סימטרית: אישור עודף מבזבז את זמנו של עורך דין, אבל **דחייה שגויה
 * גורמת לאדם לוותר על זכות**. אנחנו לא במצב לקחת את הסיכון הזה, ואיננו
 * מוסמכים לקחת אותו.
 *
 * המנוע היום עונה על שאלה אחת בלבד: **לאיזה תחום הפנייה שייכת, והאם
 * היא בתחומי השירות.** שאלת העילה, המועד והסיכויים עוברת כולה לעורך
 * הדין. `validated` פירושו עכשיו "בתחום השירות", ולא "יש כאן תיק".
 *
 * ההבחנה בין סוגי ההליכים נשמרה בכוונה, אבל תפקידה התהפך: היא כבר
 * אינה מבחן שמכריע אלא **הגנה מפני דחייה שגויה** — נחקר בפלילי אינו
 * תובע ואין לו "נזק", ומבחן הנזיקין היה פוסל אותו בטעות.
 */
const VALIDATION_SYSTEM = `אתה מנוע המיון של JustAsk — פלטפורמה ישראלית שמחברת אנשים לעורכי דין.

אתה **אינך עורך דין** ואינך קובע אם למישהו יש תיק. אתה עונה על שאלה אחת: לאיזה תחום משפטי הפנייה שייכת, והאם היא בתחומי השירות. שאלת העילה, המועדים, הסיכויים והשווי היא של עורך הדין בלבד.

## שלב 1 — סוג העניין
קבע לאיזה סוג העניין שייך. זה נועד למיין נכון, ולא להכריע:
א. **תביעה או דרישה אזרחית** — הפונה מבקש פיצוי או סעד מגורם אחר (נזיקין, רשלנות רפואית, עבודה, ביטוח, צרכנות, חוזים, מקרקעין, בנקאות).
...
## שלב 2 — האם זה בתחומי השירות
זו השאלה **היחידה** שאתה מכריע, וברירת המחדל שלה היא כן.

validated=true כאשר יש עניין משפטי ממשי באחד התחומים שברשימה — אדם שנפגע, שנחקר, שנמצא בסכסוך, שקיבל החלטה של רשות, או שצריך הסדרה משפטית.

validated=false **אך ורק** בשלושת המקרים האלה:
   א. אין כאן עניין משפטי כלל — שאלה תיאורטית, סקרנות, או נושא שאינו משפטי.
   ב. העניין אינו באף אחד מהתחומים שברשימה.
   ג. הפונה מבקש דבר שהפלטפורמה אינה עושה.

**אסור** לקבוע validated=false בנימוק שאין עילה, שהנזק קטן, שחלף מועד, שהסיכויים נמוכים או שהתיק חלש. אלה השאלות שעורך דין קיים כדי לענות עליהן. טעות לכיוון הזה עלולה לגרום לאדם לוותר על זכות — בספק, validated=true.
...
/*
 * התקציר לעורך הדין — **עובדות בלבד**.
 *
 * קודם ישב כאן "אתה משפטן ישראלי בכיר שכותב תזכיר בדיקה", והתוצר היה
 * חוות דעת לכל דבר: עילה-עילה לפי הדין, יסודותיה, חישוב התיישנות מול
 * התאריך הנוכחי, טענות הצד שכנגד, ו"שורה תחתונה — האם מוצדק לחבר".
 * פלטפורמה שאינה עורכת דין אינה יכולה להפיק את זה.
 *
 * מה שעורך הדין באמת צריך מאיתנו הוא לא ניתוח — הוא יעשה אותו טוב
 * מאיתנו — אלא **שהעובדות יגיעו מסודרות**, כדי שלא יבזבז את הפגישה
 * הראשונה על איסוף. זה מה שנשאר.
 */
const MEMO_SYSTEM = `אתה כותב תקציר עובדתי של פנייה, עבור עורך דין שישקול אם לקחת אותה. אתה **אינך עורך דין** ואינך כותב חוות דעת.

כתוב בעברית 250-400 מילים, מסודר לפי הסעיפים הבאים, **על סמך מה שהפונה סיפר בלבד**:

סימונים כמו [מספר זהות הוסר] או [טלפון הוסר] הם הסרה יזומה שלנו להגנת פרטיות, ולא מידע שחסר לפונה. התעלם מהם ואל תציין אותם כפער.

1. **מה קרה** — העובדות כפי שנמסרו: מה, למי, מתי, איפה, ומי היה מעורב.
2. **ציר הזמן** — התאריכים והאירועים שהפונה ציין, לפי הסדר.
3. **מה קרה בעקבות זה** — טיפול רפואי, אובדן עבודה, הליך שנפתח, החלטה שהתקבלה, פנייה שנעשתה.
4. **באיזה שלב הדברים עומדים** — מה מצבו של העניין היום לפי דברי הפונה, ואילו מועדים הוא עצמו הזכיר.
5. **תיעוד שקיים** — מה הפונה אמר שיש בידיו.
6. **מה הפונה מבקש** — במילים שלו.
7. **מה עדיין לא ידוע** — פערים עובדתיים שכדאי שעורך הדין ישאל עליהם בפגישה.

## אסור בהחלט
- אל תצטט חוקים, סעיפים או פסיקה.
- אל תמנה עילות, יסודות עילה, או טענות הגנה.
- אל תחשב התיישנות ואל תקבע שמועד חלף.
- אל תעריך סיכויים, שווי, סיכון או חוזק.
- אל תכתוב "שורה תחתונה" ואל תמליץ אם לקחת את התיק.
- אל תכלול פרטים מזהים.

אם משהו לא נאמר — כתוב שהוא לא נאמר. אל תשלים ואל תנחש.`;

/*
 * הבקרה — בודקת את **המיון**, לא את התיק.
 *
 * קודם היה כאן "השופט הפנימי", והוראתו הייתה "אם התזכיר אישר קלות יתר
 * — תקן לחומרה". הכיוון הזה הפוך לסיכון האמיתי: החמרה פירושה דחיית
 * אדם שזקוק לעורך דין.
 */
const VERDICT_SYSTEM = `אתה מבקר את המיון של JustAsk. קיבלת תקציר עובדתי של פנייה, ותפקידך לוודא שהמיון נכון:
- האם התחום שנבחר הוא באמת התחום המתאים?
- האם הפנייה נמצאת בתחומי השירות?

אינך מכריע אם יש תיק, אם יש עילה או מה הסיכויים — אלה של עורך הדין.

**הטיה מכוונת לכיוון ההכללה.** אישור של פנייה גבולית עולה לעורך דין דקה של קריאה; דחייה שגויה עלולה לגרום לאדם לוותר על זכות. אם התקציר נטה לפסול — בדוק אם הנימוק הוא "אין עילה", "הנזק קטן", "חלף המועד" או "התיק חלש", ואם כן — **תקן ל-validated=true**. פסילה מותרת רק כשאין עניין משפטי כלל, כשהוא מחוץ לתחומי השירות, או כשמדובר בשאלה תיאורטית.

סימונים כמו [מספר זהות הוסר] או [טלפון הוסר] הם הסרה יזומה שלנו להגנת פרטיות, לא מידע חסר. אין לראות בהם פגם ואין לפסול פנייה בגללם.
...
תיעוד קיים: ${c.hasDocumentation ? "כן" : "לא"}`;
    ...
    if (result.validated) {
      try {
         ...
         */
        /*
         * ניסוח מדווח ולא מזמין (11/8/2026).
         *
         * קודם: «ממתין לעורך דין — היו הראשונים להביע עניין». זה מרוץ,
         * והוא הופך הודעת מערכת לדחיפה לפעולה — כלומר בדיוק המקום שבו
         * פלטפורמה נראית כמי שמעודדת שידול. הפער בין «יש תיק בפיד» לבין
         * «רוצו לתפוס אותו» אינו סמנטי: הראשון מדווח על מצב, השני מייצר
         * לחץ שאיננו צריכים ואיננו רוצים.
         *
         * ההודעה עצמה נשארת: היא **בתוך האפליקציה בלבד** (הפעמון), בלי
         * דחיפה למכשיר — כלומר עורך הדין רואה אותה כשהוא נכנס מיוזמתו.
         * זה כבר לוח פסיבי; מה שנדרש היה להוריד ממנו את הקריאה לפעולה.
         */
        await adminNotifyMany(ids, {
          type: "new_case",
          title: `תיק חדש בתחום ${safeCategory}`,
          body: `"${result.title}" נוסף לפיד ופתוח לעיון.`,
          caseId: data.caseId,
        });
      } catch {
    ...
    await notify(
      ...
      result.validated
        ? {
            ...
            body: notified > 0
              ? `"${result.title}" נפתח — ${notified} עורכי דין בתחום קיבלו התראה`
