...
describe("פתיחת מקרה חדש", () => {
  ...
  it("אף מסך אינו שולח לפתיחת מקרה דרך מסך אישור התנאים", () => {
    /*
     * /onboarding הוא מסך אישור התנאים, וסופו navigate("/"). הוא נגיש
     * אך ורק בסיום ההרשמה — שם ההפניה יושבת בתוך ternary ולכן אינה
     * נחשבת יעד קבוע. כל יעד קבוע אליו הוא הבאג שדווח.
     */
    const refs = navTargets().filter((t) => t.target === "/onboarding");
    expect(
