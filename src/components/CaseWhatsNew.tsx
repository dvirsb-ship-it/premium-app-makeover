...
export function CaseWhatsNew({ caseId, status, compact = false }: { caseId: string; status: Case["status"]; compact?: boolean }) {
  ...
  if (compact) {
    return (
      <span className={cn("mt-1 flex items-center gap-1.5 text-[11px] font-bold", bright ? "text-gold-ink dark:text-gold" : "text-foreground/70")}>
        <span aria-hidden className={cn("size-1.5 shrink-0 rounded-full", bright ? "bg-gold-ink dark:bg-gold" : "bg-muted-foreground/50")} />
