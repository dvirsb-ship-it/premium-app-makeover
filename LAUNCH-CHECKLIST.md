...
## איך בודקים — פקודה אחת

```
LAUNCH=1 bun run test
```

`src/lib/launch-gate.test.ts` נכשל על כל דגל שאינו במצב ההשקה שלו, עם
הסבר למה הוא חשוב. ביום רגיל הבדיקה רק מתעדת את המצב ולא מפריעה.

**זה הופך את המסמך הזה למנגנון.** מי שמשלח בלי לעבור את השער חייב
לכבות אותו במפורש — וזו פעולה מודעת, לא שכחה.
...
- [x] ~~למחוק את תיקי הבדיקה~~ — **נבדק על מכשיר 17/8/2026 ואינו רלוונטי.** הסט מגוון: התעניינות, חיבור, עבר בדיקה, לא זוהה סיכוי. הם **כן** ייספרו מול המכסה ביום שתידלק, ולכן נמחקים אז — לא לפני
